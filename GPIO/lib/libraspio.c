...
/** Reads a pin and counts for a number of cycles before returning a 1 
 * 	if it considers that the button has been pressed. Run in a thread.
 * \param pin_button the pin connected to the button
 * \param trigger_time the time to wait before deciding if the button has been pressed
 * \return a 1 if the button is considered "pressed" a 0 
*/
int button(short pin_button, int trigger_time) {
	int counter = 0;
	int enable = 1;
	// Bounce back and hold time algorithm
	while (enable) {
		while(digitalRead(pin_button)) {
			counter++;
			if (counter > trigger_time*1000) {
				counter = 0;
				enable = 0;
				return 1;
			}
